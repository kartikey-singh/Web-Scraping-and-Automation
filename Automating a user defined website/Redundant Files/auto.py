...
def automation():
	...
	for i in range(0,headers.shape[0]):
		...
		path = os.getcwd()
		path_browesermob = path + "\\browsermob-proxy\\bin\\browsermob-proxy.bat"
		# path1 = 'C:\\Users\\Kartikey\\Downloads\\WebScrapingFormFilling-master\\WebScrapingFormFilling-master\\browsermob-proxy\\bin\\browsermob-proxy.bat'
		server = Server(path=path_browesermob, options=dict)
		server.start()
		time.sleep(1)
		...
		# path2 = 'C:\\Users\\Kartikey\\Downloads\\WebScrapingFormFilling-master\\WebScrapingFormFilling-master\\Drivers\\chromedriver.exe'
		chrome_options = webdriver.ChromeOptions()
		chrome_options.add_argument("--proxy-server={0}".format(proxy.proxy)) #Configure chrome options
		driver = webdriver.Chrome(chrome_options=chrome_options,executable_path=path_chromedriver)
